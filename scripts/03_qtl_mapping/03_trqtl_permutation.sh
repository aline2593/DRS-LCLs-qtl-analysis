...
#
# Differs from eQTL:
#   - Uses transcript expression (not gene expression)
#   - Tests isoform-level associations
#   - May find isoform-switching QTLs missed by gene-level analysis
#
# Approach:
...
#
# Input:  - Transcript expression BED (from FLAIR/Bambu/StringTie)
...
################################################################################
# STEP 1: CALCULATE TRANSCRIPT EXPRESSION PCs
################################################################################

echo "STEP 1: Calculating transcript expression principal components..."
echo "----------------------------------------------------------------------"
...
if [[ ! -f "${TRANSCRIPT_BED}.tbi" ]]; then
    echo "Creating index for transcript BED..."
    tabix -p bed "${TRANSCRIPT_BED}"
fi

# Calculate PCs from transcript expression data
bsub -P "$PROJECT" -q "$QUEUE" -n 1 -W 20:10 \
    -o "$OUTPUT_DIR/logs/pca.out" \
    -e "$OUTPUT_DIR/logs/pca.err" \
    "QTLtools pca --bed ${TRANSCRIPT_BED} --scale --center --out ${OUTPUT_DIR}/transcript_expression_pcs"

echo "Waiting for PCA job to complete..."
echo "Check: $OUTPUT_DIR/transcript_expression_pcs.pca"
echo ""
...
# Combine base covariates with transcript expression PCs
cat "${BASE_COV}" > "${OUTPUT_DIR}/merged_covariates_base.txt"
cat "${OUTPUT_DIR}/transcript_expression_pcs.pca" >> "${OUTPUT_DIR}/merged_covariates_base.txt"
...
# Transcript expression PCs (pattern may differ from gene PCs)
pc_rows <- grep("^genes.*PC[0-9]+$", cov_full$Sample, value = TRUE)

cat("Total transcript expression PCs available:", length(pc_rows), "\n")
...
for (f in list.files(pattern = "trQTL_perm_.*PC_merged\\.txt\\.gz$")) {
  ...
  # Count significant eTranscripts at FDR < 5%
  n_sig <- sum(q$qvalues <= 0.05, na.rm = TRUE)

  # Store results
  numpcs <- rbind(numpcs, data.frame(
    n_pcs = pc_num,
    n_etranscripts_fdr5 = n_sig,
    pi1 = round(1 - q$pi0, 3)
  ))

  cat("  PCs:", pc_num, "| eTranscripts (FDR<5%):", n_sig, "| π₁:", round(1 - q$pi0, 3), "\n")
}
...
# Plot 1: Number of eTranscripts vs PCs
plot(numpcs$n_pcs, numpcs$n_etranscripts_fdr5,
     type = "b",
     pch = 19,
     col = "darkblue",
     lwd = 2,
